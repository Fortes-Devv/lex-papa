...
const INITIAL_FAVORITES = [MOCK_PRODUCTS[0].id, MOCK_PRODUCTS[2].id, MOCK_PRODUCTS[4]?.id].filter(Boolean) as string[];
      ...
      {filtered.length > 0 ? (
            ...
            <div key={product.id} className="group rounded-lg border border-border bg-card overflow-hidden hover:border-primary/30 hover:shadow-md transition-all duration-200 flex flex-col">
              <div className="relative aspect-video overflow-hidden bg-muted">
                ...
                <button
                  ...
                  className="absolute top-2 right-2 h-7 w-7 rounded-full bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-destructive"
              ...
              <div className="p-4 flex flex-col flex-1 space-y-2">
                ...
                <div className="flex items-center justify-between pt-2 border-t border-border">
                  ...
                  <div className="flex gap-1.5">
                    <Button size="xs" variant="ghost" onClick={() => removeFavorite(product.id)}>
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <Link href="/student/explore">
            <Button variant="outline" size="sm" className="mt-4" leftIcon={<BookOpen className="h-4 w-4" />}>
              Explorar cursos
            </Button>
          </Link>
        </div>
