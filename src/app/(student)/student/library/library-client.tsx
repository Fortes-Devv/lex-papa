...
import { Search, BookOpen, CheckCircle2, Clock } from "lucide-react";
...
export function LibraryClient({ enrollments }: { enrollments: LibraryEnrollment[] }) {
  const [search, setSearch] = useState("");
  const active = enrollments.filter((e) => e.progress < 100);
  const completed = enrollments.filter((e) => e.progress === 100);

  const filtered = (list: LibraryEnrollment[]) =>
    list.filter((e) => !search || e.title.toLowerCase().includes(search.toLowerCase()));

  function CourseCard({ enrollment }: { enrollment: LibraryEnrollment }) {
    const isComplete = enrollment.progress === 100;
    const playerHref = enrollment.courseId ? `/student/player?courseId=${enrollment.courseId}` : "/student/library";
    return (
      <div className="group rounded-lg border border-border bg-card overflow-hidden hover:border-primary/30 hover:shadow-md transition-all duration-200">
        ...
        <div className="p-4 space-y-3">
          <h3 className="text-sm font-semibold text-foreground line-clamp-2 leading-snug">{enrollment.title}</h3>
